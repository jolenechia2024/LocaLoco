...
class businessController {
    static async getAllBusinesses(
        ...
        try {
            ...
            console.error(
                `There was a problem fetching the businesses: ${error}`,
            );
            next(error);
        }
    }

    static async getFilteredBusinesses(
        req: Request,
        res: Response,
        next: NextFunction,
    ): Promise<void> {
        try {
            const filteredBusinesses =
                await BusinessModel.getFilteredBusinesses(req.body);
            res.status(200).json(filteredBusinesses);
        } catch (error) {
            console.error(
                `There was a problem fetching the filtered businesses: ${error}`,
            );
            next(error);
        }
    }

    static async getBusinessByUEN(
        req: Request,
        res: Response,
        next: NextFunction,
    ): Promise<void> {
        try {
            const business = await BusinessModel.getBusinessByUEN(
                String(req.query.uen),
            );
            res.status(200).json(business);
        } catch (error) {
            console.error(
                `There was a problem fetching the selected business: ${error}`,
            );
            next(error);
        }
    }
    ...
    static async searchBusinessByName(
        ...
        try {
            ...
            const business =
                await BusinessModel.searchBusinessByName(searchName);
            ...
            console.error(
                `There was a problem searching for business: ${error}`,
            );
            next(error);
        }
    }

    static async getOwnedBusinesses(
        req: Request,
        res: Response,
        next: NextFunction,
    ): Promise<void> {
        try {
            const ownedBusinesses = await BusinessModel.getOwnedBusinesses(
                String(req.body.ownerId),
            );
            res.status(200).json(ownedBusinesses);
        } catch (error) {
            console.error(
                `There was a problem fetching the the owned business: ${error}`,
            );
            next(error);
        }
    }
    ...
    static async registerBusiness(
        ...
        try {
            ...
            const ownerEmail = (await UserModel.getUserById(business.ownerId))
                .profile!.email;
            ...
            console.error(
                `There was a problem registering the selected business: ${err}`,
            );
    ...
    static async updateBusiness(
        ...
        if (!ownerId) {
            res.status(400).json({
                error: "Owner ID is required for business update",
            });
            return;
        }
        ...
        try {
            ...
            const existingBusiness =
                await BusinessModel.getBusinessByOwnerId(ownerId);
            ...
            const business = {
                ...
                businessName: req.body.businessName,
                businessCategory:
                    req.body.businessCategory || req.body.category,
                description: req.body.description,
                address: req.body.address,
                ...
                socialMediaLink:
                    req.body.socialMediaLink || req.body.socialMedia || "",
                wallpaper: req.body.wallpaper,
                priceTier: req.body.priceTier,
                offersDelivery: req.body.offersDelivery ? 1 : 0,
                offersPickup: req.body.offersPickup ? 1 : 0,
            ...
            const updatedBusiness = await BusinessModel.getBusinessByUEN(
                business.uen,
            );
            ...
            console.error(
                `There was a problem updating the selected business: ${err}`,
            );
            next(err);
        }
    }
    ...
    static async deleteBusiness(
        ...
        try {
            ...
            console.error(
                `There was a problem deleting the selected business: ${err}`,
            );
            next(err);
        }
    }

    static async checkUenAvailability(
        req: Request,
        res: Response,
        next: NextFunction,
    ): Promise<void> {
