...
userRouter.get(
    "/api/users/auth-provider/:userId",
    UserController.getAuthProvider.bind(UserController),
);
...
userRouter.post(
    "/api/user/referral",
    UserController.handleReferral.bind(UserController),
);

// Get user vouchers
userRouter.get(
    "/api/users/:userId/vouchers",
    UserController.getUserVouchers.bind(UserController),
);

// update the status of the user's voucher/s
userRouter.post(
    "/api/user/update-voucher",
    UserController.updateVoucherStatus.bind(UserController),
);

// Redeem voucher with loyalty points
userRouter.post(
    "/api/user/redeem-voucher",
    UserController.redeemVoucher.bind(UserController),
);
