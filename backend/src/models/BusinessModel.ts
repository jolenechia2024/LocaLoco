...
class BusinessModel {
    public static async getAllBusinesses() {
        ...
        for (const business of businessRows) {
            ...
            const openingHours: Record<DayOfWeek, HourEntry> = {} as Record<
                DayOfWeek,
                HourEntry
            >;
            ...
            if (!business.open247) {
                ...
                for (const h of hourRows) {
                    openingHours[h.dayOfWeek as DayOfWeek] = {
                        open: h.openTime,
                        close: h.closeTime,
                    };
                }
    ...
    public static async getBusinessByUEN(
        uen: string,
        ...
        const paymentRow = await db
            .select()
            .from(businessPaymentOptions)
            .where(eq(businessPaymentOptions.uen, uen));
    ...
    public static async getFilteredBusinesses(
        ...
        if (filters.price_tier) {
            if (
                Array.isArray(filters.price_tier) &&
                filters.price_tier.length > 0
            ) {
                conditions.push(
                    inArray(businesses.priceTier, filters.price_tier),
                );
        ...
        try {
            let query = db.select().from(businesses).$dynamic();
            ...
            switch (filters.sort_by) {
                case "business_name":
                    query = query.orderBy(
                        sortDirection(businesses.businessName),
                    );
                    break;
                case "price_tier":
                ...
                default:
                    query = query.orderBy(
                        sortDirection(businesses.dateOfCreation),
                    );
                    break;
            }
            ...
            const allOpeningHours =
                nonOpen247Uens.length > 0
                    ? await db
                          ...
                          .from(businessOpeningHours)
                          .where(
                              inArray(businessOpeningHours.uen, nonOpen247Uens),
                          )
                    : [];
            ...
            const fullBusinesses: Business[] = businessRows.map((business) => ({
                ...
                open247: Boolean(business.open247),
                openingHours:
                    openingHoursMap.get(business.uen) ||
                    ({} as Record<DayOfWeek, HourEntry>),
                email: business.email!,
                phoneNumber: business.phoneNumber!,
                websiteLink: business.websiteLink ?? null,
    ...
    public static async registerBusiness(business: Business) {
        try {
            ...
            if (!business.open247 && business.openingHours) {
                const openingHourEntries = Object.entries(
                    business.openingHours,
                ) as [DayOfWeek, HourEntry][];
                await Promise.all(
    ...
    public static async searchBusinessByName(
        searchName: string,
    ): Promise<{ uen: string; name: string } | null> {
        ...
        const partialMatch = await db
            ...
            .from(businesses)
            .where(
                sql`LOWER(${businesses.businessName}) LIKE ${"%" + sanitized + "%"}`,
            )
            .limit(1);
        ...
        for (const business of allBusinesses) {
            const businessNameLower = business.name
                .toLowerCase()
                .replace(/[^\w\s'-]/g, "")
                .replace(/\s+/g, " ")
                .trim();
    ...
    public static async getBusinessByOwnerId(
        ownerId: string,
    ): Promise<BusinessForBusinessList | null> {
        try {
            const result = await db
                .select()
                .from(businesses)
            ...
            const paymentOptionsResult = await db
                .select()
                .from(businessPaymentOptions)
                .where(eq(businessPaymentOptions.uen, business.uen));
            const paymentOptions = paymentOptionsResult.map(
                (po) => po.paymentOption,
            );
