...
class UserModel {
        ...
        try {
            ...
            if (profile[0]) {
                const availablePoints = await db
                    .select()
                    .from(userPoints)
                    .where(eq(userPoints.userEmail, profile[0].email));
                ...
                reviews = await db
                    ...
                    .where(eq(businessReviews.userEmail, profile[0].email));
    ...
    public static async updateProfile(
        ...
        try {
            ...
            if (updates.imageUrl !== undefined)
                updateData.image = updates.imageUrl;
            if (updates.bio !== undefined) updateData.bio = updates.bio;
            if (updates.hasBusiness !== undefined)
                updateData.hasBusiness = updates.hasBusiness;
    ...
    public static async handleReferral(
        referralCode: string,
        referredId: string,
    ) {
        try {
            ...
            const referredUserCheck = await db
                .select({
     ...
     */
    public static async getAuthProvider(
        userId: string,
    ): Promise<string | null> {
        try {
